...
mod tests {
    ...
    fn role_test() -> Result<(), String> {
        ...
        let mut principal_signer: Principal = Principal::anonymous();
        ...
        for _i in 0..n {
            ...
            principal_signer = generate_random_principal();
            assert!(!has_role(UserRole::Signer, principal_signer));
            let _res = add_role_internal(UserRole::Signer, principal_signer);
            assert!(has_role(UserRole::Signer, principal_signer));
        }

        let _users = users_of_role(UserRole::Admin);
        assert_eq!(_users.len(), n as usize);
        let _users = users_of_role(UserRole::Signer);
        assert_eq!(_users.len(), n as usize);

        _remove_role(UserRole::Admin, principal_admin);
        assert!(!has_role(UserRole::Admin, principal_admin));
        _remove_role(UserRole::Signer, principal_signer);
        assert!(!has_role(UserRole::Signer, principal_signer));
        ...
        _clear_users_of_role(UserRole::Signer);
        let _users = users_of_role(UserRole::Signer);
        assert_eq!(_users.len(), 0);
        Ok(())
    }
